// --------------------------------------------------------------------------------------------------------------------
// <copyright file="ElementResolver.cs" company="Solidsoft Reply Ltd">
...
// </copyright>
// <summary>
// Resolves GS1 data elements, validating the element value and providing descriptors.
// </summary>
// --------------------------------------------------------------------------------------------------------------------
...
using Solidsoft.Reply.Parsers.Gs1Ai.Descriptors;
...
internal static partial class ElementResolver {
#else
internal static class ElementResolver {
   ...
   /// <summary>
    ...
    /// </summary>
    private static readonly IDictionary<int, ElementDescriptors> Descriptors =
        new Dictionary<int, ElementDescriptors>
        {
            {
...
#pragma warning restore SA1115 // Parameter should follow comma
...
#if NET7_0_OR_GREATER
    /// <summary>
    ///     Resolve a first two digits of the application identifier into a data element.
    /// </summary>
    /// <param name="data">
    ///     The data buffer.
    /// </param>
    ...
    /// </param>
    /// <returns>
    ///     A <see cref="ResolvedApplicationIdentifierRef" /> representing a resolved data element.
    /// </returns>
    public static ResolvedApplicationIdentifierRef ResolveEx(
        ...
        ResolveElement(dataCopy, firstTwoDigits, currentPosition, identifier, value, parameters);
        value = value.TrimEnd('\0');
        ApplicationIdentifier elementAi = (ApplicationIdentifier)parameters[0];
        int numberOfDecimalPlaces = parameters[1];
        int sequenceNumber = parameters[2];

        if (!includeDescriptors) {
            aiRef.Entity = (int)elementAi;
            identifier.GetIdentifierIfNull((int)elementAi).CopyTo(aiRef.Identifier);
        ...
        var descriptors = elementAi.GetDescriptors();

        elementAi = descriptors.DataTitle == string.Empty
            ? ApplicationIdentifier.Unrecognised
            : elementAi;

        if (elementAi == ApplicationIdentifier.Unrecognised) {
            aiRef.Entity = -1;
    ...
    /// </param>
    /// <returns>
    ///     A <see cref="ResolvedApplicationIdentifier" /> representing a resolved data element.
    /// </returns>
    public static ResolvedApplicationIdentifier Resolve(
        ...
        int index,
        bool includeDescriptors = true) =>
            Resolve(data.AsSpan(), firstTwoDigits.AsSpan(), currentPosition, index, includeDescriptors);
        ...
        if (!includeDescriptors) {
            return Validate(
                new ResolvedApplicationIdentifier(
                    (int)elementAi,
                    identifier.GetIdentifierIfNull((int)elementAi).ToString(),
    ...
    private static void ResolveElement(
        ...
        Span<char> value,
        Span<int> parameters) {
        ApplicationIdentifier elementAi;
        ...
        switch (firstTwoDigits) {
            ...
            case "98":
            case "99":
                elementAi = data.GetElement2();
                extractedValue = data.GetValue(2);
                break;
            case "23":
                // 5
                elementAi = data.IsNumberEqual(2, 1, 5) ? data.GetElement3() : ApplicationIdentifier.Unrecognised;
                if (elementAi == ApplicationIdentifier.Unrecognised) {
                    extractedValue = data.GetValue(2);
                }
                else {
            ...
            case "25":
                // 0..1, 3..5
                elementAi = data.IsDigitInRange(2, 1) || data.IsNumberInRange(2, 1, 3, 5)
                             ? data.GetElement3()
                             : ApplicationIdentifier.Unrecognised;
                if (elementAi == ApplicationIdentifier.Unrecognised) {
                    extractedValue = data.GetValue(2);
            ...
            case "40":
                // 0..3
                elementAi = data.IsDigitInRange(2, 3) ? data.GetElement3() : ApplicationIdentifier.Unrecognised;
                if (elementAi == ApplicationIdentifier.Unrecognised) {
                    extractedValue = data.GetValue(2);
                }
                else {
            ...
            case "71":
                // 0..7
                elementAi = data.IsNumberInRange(2, 1, 0, 7)
                             ? data.GetElement3()
                             : ApplicationIdentifier.Unrecognised;
                if (elementAi == ApplicationIdentifier.Unrecognised) {
                    extractedValue = data.GetValue(2);
            ...
            case "42":
                // 0..7
                elementAi = data.IsDigitInRange(2, 7) ? data.GetElement3() : ApplicationIdentifier.Unrecognised;
                if (elementAi == ApplicationIdentifier.Unrecognised) {
                    extractedValue = data.GetValue(2);
                }
                else {
            ...
            case "39":
                // 0n..5n
                elementAi = data.IsDigitInRange(2, 5) ? data.GetElement3() : ApplicationIdentifier.Unrecognised;
                if (elementAi == ApplicationIdentifier.Unrecognised) {
                    extractedValue = data.GetValue(2);
                }
                else {
                    identifier = ((int)data.GetElement4()).ToInvariantSpan(identifier, "##00");
            ...
            case "31":
                // 0n..6n
                elementAi = data.IsDigitInRange(2, 6) ? data.GetElement3() : ApplicationIdentifier.Unrecognised;
                if (elementAi == ApplicationIdentifier.Unrecognised) {
                    extractedValue = data.GetValue(2);
                }
                else {
            ...
            case "36":
                // 0n..9n
                elementAi = data.IsDigitInRange(2, 9) ? data.GetElement3() : ApplicationIdentifier.Unrecognised;
                if (elementAi == ApplicationIdentifier.Unrecognised) {
                    extractedValue = data.GetValue(2);
                }
                else {
            ...
            case "43":
                // 00..26, 30..33
                elementAi = data.IsNumberInRange(2, 2, 0, 26) || data.IsNumberInRange(2, 2, 30, 33)
                    ? data.GetElement4()
                    : ApplicationIdentifier.Unrecognised;
            ...
            case "70":
                ...
                if (elementAi == ApplicationIdentifier.Unrecognised) {
                    ...
                    if (elementAi == ApplicationIdentifier.NumberOfProcessorWithIsoCountryCode) {
                        identifier = ((int)data.GetElement4()).ToInvariantSpan(identifier, "##00");
            ...
            case "72":
                // 3s, 40, 50-59
                elementAi = data.IsNumberEqual(2, 1, 3)
                ...
                if (elementAi == ApplicationIdentifier.Unrecognised) {
                    ...
                    if (elementAi == ApplicationIdentifier.CertificationReference) {
                        identifier = ((int)data.GetElement4()).ToInvariantSpan(identifier, "##00");
                        sequenceNumber = data.GetSequenceNumber(3);
                    }
            ...
            case "80":
                ...
                elementAi = data.IsNumberInRange(2, 2, 1, 9) || data.IsNumberInRange(2, 2, 10, 14)
            ...
            case "81":
                // 10..12
                elementAi = data.IsNumberInRange(2, 2, 10, 12)
                             ? data.GetElement4()
                             : ApplicationIdentifier.Unrecognised;
                if (elementAi == ApplicationIdentifier.Unrecognised) {
                    extractedValue = data.GetValue(2);
            ...
            case "82":
                // 00
                elementAi = data.IsNumberEqual(2, 2, 0) ? data.GetElement4() : ApplicationIdentifier.Unrecognised;
                if (elementAi == ApplicationIdentifier.Unrecognised) {
                    extractedValue = data.GetValue(2);
                }
                else {
                ...
                break;
            default:
                elementAi = ApplicationIdentifier.Unrecognised;
                extractedValue = data.GetValue(2);
                break;
        }
    ...
    /// </summary>
    /// <param name="elementAi">The application identifier of the element.</param>
    /// <returns>The descriptors tuple for the given application identifier.</returns>
    private static ElementDescriptors GetDescriptors(this ApplicationIdentifier elementAi) {
        return elementAi == ApplicationIdentifier.Unrecognised
                   ? new ElementDescriptors(null, null, null, false)
                   : GetDescriptor();

        // In highly unusual circumstances, it is possible to obtain an invalid element
        ElementDescriptors GetDescriptor() =>
            Descriptors.TryGetValue((int)elementAi, out var descriptor)
                ? descriptor
                : new ElementDescriptors(string.Empty, null, null, false);
    }

    /// <summary>
    ///     Returns an element from the given data based on a given number of characters.
    /// </summary>
    /// <param name="data">
    ///     The data containing the element identifier.
    /// </param>
    /// <param name="length">
    ///     The length of the element identifier.
    /// </param>
    /// <returns>
    ///     An element.
    /// </returns>
    private static ApplicationIdentifier GetElement(this ReadOnlySpan<char> data, int length) {
...
#pragma warning restore SA1114 // Parameter list should follow declaration
    ...
    /// <summary>
    ///     Returns an element from the given data based on the first two characters.
    /// </summary>
    /// <param name="data">The data containing the element's application identifier.</param>
    /// <returns>An element application identifier.</returns>
    private static ApplicationIdentifier GetElement2(this ReadOnlySpan<char> data) {
        return GetElement(data, 2);
    }

    /// <summary>
    ///     Returns an element from the given data based on the first three characters.
    /// </summary>
    /// <param name="data">The data containing the element's application identifier.</param>
    /// <returns>An element application identifier.</returns>
    private static ApplicationIdentifier GetElement3(this ReadOnlySpan<char> data) {
        return GetElement(data, 3);
    }

    /// <summary>
    ///     Returns an element from the given data based on the first four characters.
    /// </summary>
    /// <param name="data">The data containing the element's application identifier.</param>
    /// <returns>An element application identifier.</returns>
    private static ApplicationIdentifier GetElement4(this ReadOnlySpan<char> data) {
        return GetElement(data, 4);
    }
...
#pragma warning restore SA1114 // Parameter list should follow declaration
    ...
    /// <summary>
    ///     Validates a resolved element.
    /// </summary>
    /// <param name="resolvedElement">The resolved element to be validated.</param>
    /// <returns>A resolved element object. If the value is invalid, the object records the error.</returns>
    private static ResolvedApplicationIdentifier Validate(ResolvedApplicationIdentifier resolvedElement) {
        try {
            var identifier =
                Descriptors[resolvedElement.Entity].IsValid(
                    resolvedElement,
                    out var validationErrors)
                    ? resolvedElement
                    : new ResolvedApplicationIdentifier(
                        new ParserException(
                            resolvedElement.Identifier.Trim(),
                            2006,
                            string.Format(
                                CultureInfo.CurrentCulture,
                                Resources.GS1_Error_006,
                                resolvedElement.Value.Length > 0 ? " " + resolvedElement.Value : string.Empty,
                                resolvedElement.Identifier.Trim()),
                            true,
                            resolvedElement.Identifier.Length + resolvedElement.Value.Length - 1),
                        resolvedElement.CharacterPosition,
                        resolvedElement);
            ...
            return new ResolvedApplicationIdentifier(
                new ParserException(
                    ...
                    string.Format(
                        ...
                        resolvedElement.Identifier.Trim()),
                    true,
                    resolvedElement.Identifier.Length - 1),
                resolvedElement.CharacterPosition,
                resolvedElement);
        }
    ...
    /// <param name="resolvedElement">The resolved element to be validated.</param>
    /// <returns>A resolved element object. If the value is invalid, the object records the error.</returns>
    private static ResolvedApplicationIdentifierRef Validate(ResolvedApplicationIdentifierRef resolvedElement) {
        try {
            var identifier =
                Descriptors[resolvedElement.Entity].IsValid(
                    resolvedElement,
                    ...
                    : new ResolvedApplicationIdentifierRef(
                        new ParserException(
                            resolvedElement.Identifier.ToString().Trim(),
                            2006,
                            string.Format(
                                CultureInfo.CurrentCulture,
                                Resources.GS1_Error_006,
                                resolvedElement.Value.Length > 0 ? " " + resolvedElement.Value.ToString() : string.Empty,
                                resolvedElement.Identifier.ToString().Trim()),
                            true,
                            resolvedElement.Identifier.Length + resolvedElement.Value.Length - 1),
                        resolvedElement.CharacterPosition,
