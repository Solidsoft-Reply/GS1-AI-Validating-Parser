...
Scenario: Trigger an 'entity is incorrectly terminated with an FNC1' error
	Given a request to parse data
	When the AI of 01 is incorrectly terminated with an FNC1 and the value is 12345678901231 
	Then the entity should be 01 
	    And there should be errors
	    And the errors should include a non-fatal 2004 error

Scenario: Trigger a 'predefined-length entity does not contain sufficient number of characters' error
...
Scenario: Trigger a 'No entity value provided for AI' error
	...
	Then the entity should be 01 
	    And there should be errors
	    And the errors should include a fatal 2007 error
...
Scenario: Trigger a 'the GTIN value{0} has an invalid check digit' error
	...
	Then the entity should be 01 
	    And there should be errors
	    And the errors should include a fatal 2009 error

Scenario: 2 Trigger a 'the GTIN value{0} has an invalid check digit' error
...
Scenario: 3 Trigger a 'the GTIN value{0} has an invalid check digit' error
...
Scenario: Trigger a 'the value{0} does not match the specified pattern for the data element' error
	...
	Then the entity should be 01 
		And there should be errors
		And the errors should include a fatal 2100 error
...
Scenario: Trigger a 'the implied decimal point position (inverse exponent) is specified incorrectly' error
	...
	Then the entity should be 310
	    And there should be errors
	    And the errors should include a fatal 2010 error